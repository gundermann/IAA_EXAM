...
<s:form>
	...
	<display:table id="publicationTable" name="publicationList"
		...
		<display:setProperty name="basic.msg.empty_list"
			value="Keine Publikationen gefunden" />
		<display:column>
			<input type="radio" name="publicationId"
				value="${publicationTable.publicationId}" />
		</display:column>
		<display:column title="Nak-ID" property="nakId" sortable="true" />
		<display:column title="Titel" property="title" sortable="true" />
		<display:column title="Datum der Veröffentlichung"
			property="dateOfPublication" sortable="true"
			format="{0,date,dd.MM.yyyy}" />
		<display:column title="Verlag" property="publisher.publisherName"
			sortable="true" />
		<display:column title="Publikationart" property="publicationType.name"
			sortable="true" />
